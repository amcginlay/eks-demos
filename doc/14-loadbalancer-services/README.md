...
kubectl -n ${EKS_NS_BLUE} patch service ${EKS_APP} --patch '{"spec": {"type": "LoadBalancer"}}' # extend the service to expose a new AWS Load Balancer
sleep 5 && kubectl -n ${EKS_NS_BLUE} get services
```
...
```bash
clb_dnsname=$(kubectl -n ${EKS_NS_BLUE} get service ${EKS_APP} -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
