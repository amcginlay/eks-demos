...
Perform a `dig` operation to observe the private mapping from DNS name to ClusterIP address.
```bash
kubectl exec -it jumpbox -- /bin/bash -c "apt-get update && apt-get install dnsutils -y && dig +short ${EKS_APP_NAME}.${EKS_NS_BLUE}.svc.cluster.local"
```
