...
We can deploy [nginx](https://www.nginx.com) as a standalone pod in the default namespace which conveniently suits this purpose however it should be noted, in the absence of an associated deployment object, it will not be replaced in the event of a failure.
```bash
