...
With the `kubectl run` command we can deploy [nginx](https://www.nginx.com) as a standalone pod which can conveniently serve as a jumpbox however it should be noted, in the absence of an associated deployment object, it will not be replaced in the event of a failure.
```bash
