...
kubectl -n ${EKS_APP_NS} expose deployment ${EKS_APP_NAME} --port=80 --type=NodePort # this will auto-assign a high-order port on ALL worker nodes
...
node_port=$(kubectl -n ${EKS_APP_NS} get service -l app=${EKS_APP_NAME} -o jsonpath='{.items[0].spec.ports[0].nodePort}')
```
